...
function ProjectComp() {
  ...
  return (
    <Fragment>
      {isWideScreen ? <DesktopComponent /> : <MobileComponent />}
    </Fragment>
  );
}
...
const DesktopComponent = () => {
  ...
  const handleViewAllClick = () => {
    // This function should handle the action when "View All" is clicked
              ...
              return (
                <div className={styles.scrollsection} key={index}>
                  <div className={styles.scrollsectiondiv}>
                    <a href={item.link} className={styles.githubLink}>
                      <h3>{item.name}</h3>
                    </a>
                  </div>
